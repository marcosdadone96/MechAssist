...
const start = lines.findIndex((l) => l.startsWith('const COMP_PRESETS'));
const end = lines.findIndex((l, i) => i > start && l.startsWith('function fmt'));
if (start < 0 || end < 0) {
  console.error('Could not find COMP_PRESETS block', start, end);
  process.exit(1);
}

const block = `const COMP_PRESETS = [
...
let s = [...lines.slice(0, start), ...block.split('\n'), ...lines.slice(end)].join('\n');
