...
export function supabaseSessionSyncErrorMessage(reason, detail) {
  ...
  if (detail && /invalid api key/i.test(detail)) {
    return en
      ...
      : 'Clave API de Supabase incorrecta en el servidor: en Netlify configure NEXT_PUBLIC_SUPABASE_URL y NEXT_PUBLIC_SUPABASE_ANON_KEY con el mismo proyecto que la web (Supabase ? Settings ? API) y vuelva a desplegar. Cerrar sesiÛn no lo soluciona.';
  }
