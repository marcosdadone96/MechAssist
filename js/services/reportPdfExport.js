...
export function buildFlatPdfPayload(raw, r) {
  ...
  return {
    ...
    disclaimer:
      'Documento generado automáticamente por TheMechAssist. Los datos de motorreductor son de catálogo de demostración. No sustituye proyecto ejecutivo, normativa aplicable ni validación del fabricante. Revise siempre fichas técnicas y condiciones de servicio (S1, temperatura, ciclo, freno, etc.).',
  };
}
...
export function buildInclinedPdfPayload(raw, r) {
  ...
  return {
    ...
    disclaimer:
      'Documento generado automáticamente por TheMechAssist. Datos de motorreductor de demostración. Valide con fabricante y normativa. En pendiente revise seguridad (freno, anti-retorno, paradas).',
  };
}
...
export function buildPumpPdfPayload(raw, r) {
  ...
  return {
    ...
    verdict: Number.isFinite(r.requiredMotorPower_kW) ? 'APTO' : 'NO APTO',
    disclaimer:
      'Documento generado automáticamente por TheMechAssist. Modelo de punto único (Q,H,η); no sustituye curvas del fabricante, NPSH ni proyecto de tuberías. Datos de motorreductor de demostración.',
  };
}
...
export function buildScrewPdfPayload(raw, r) {
  ...
  return {
    ...
    disclaimer: en
      ? 'Auto-generated by TheMechAssist. Indicative screw model; does not replace full CEMA 350 or OEM data. Check max RPM and wear with the equipment supplier.'
      : 'Documento generado autom\u00e1ticamente por TheMechAssist. Modelo orientativo de tornillo; no sustituye CEMA 350 completo ni datos del fabricante. Revise RPM m\u00e1ximas y desgaste con el proveedor del equipo.',
  };
}
...
export function buildRollerPdfPayload(raw, r) {
  ...
  return {
    ...
    verdict: Number.isFinite(r.requiredMotorPower_kW) ? 'APTO' : 'NO APTO',
    disclaimer:
      'Documento generado automáticamente por TheMechAssist. Modelo orientativo para línea de rodillos, sujeto a validación final de fabricante e instalación real.',
  };
}
