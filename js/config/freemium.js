 ...
 * M�quinas: cinta plana, inclinada y rodillos con c�lculo completo sin Pro; el resto del hub m�quinas es Pro.
 * En esas 3, guardar configuraci�n e informe PDF siguen siendo Pro (ver machineConfigMount, reportPdfExport).
 * Laboratorio (transmission-canvas) sigue en Pro. El resto del sitio no listado aqu� se trata como gratis.
 */

/** M�dulos con c�lculo y acordeones abiertos sin Pro (no aplican `applyMachinePremiumGates` de acorde�n). */
...
/** Mismas rutas sin extensi�n (pretty URLs, rewrites). Incluye alias con guion bajo por servidores locales. */
 ...
 * Configuraci�n guardada e informe PDF no dependen de esta funci�n; requieren `isPremiumEffective()`.
 * Acepta pathname, URL completa o se eval�a `window.location` si existe.
 *
 * @param {string} [hrefOrPath]
 * @returns {boolean}
 */
